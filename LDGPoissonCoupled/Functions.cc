...
template <int dim>
double distance(Point<dim> point1, Point<dim> point2)
{
  double d = 0;
...
{
 // std::cout<<"rhs omega dim "<<dim<<std::endl;
  switch (constructed_solution)
  {
  case 1:
  {
             ...
             Vector<double> &values) const
{
  ...
  case 1:
  {
    if(dim == 2)
      ...
      values(3) = w *std::sin(w * x) * std::cos(w * y_l) * std::cos(w * z_l);//q
    ...
    case 2:
    {
      ...
      values(0) = 0; //Q
      values(1) = 0;
      values(2) = r > radius ? values(4) * g/(1 +g) * (1 - radius * std::log(r/radius)) : values(4) * g/(1 +g) ; // U 
      values(3) = 0; //q
      ...
      values(2) = r > radius ?  - g/(g+1) * (radius*(z - z_l)/std::pow(r,2)) * values(5) : 0;
      
      values(4) = r > radius ? values(5) * g/(1 +g) * (1 - radius * std::log(r/radius)) : values(5) * g/(1 +g);      // U  
      }
    break;
    }
    case 3:
