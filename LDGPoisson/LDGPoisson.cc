...
make_grid()
{
const double half_length =  std::sqrt(0.5);
Point<dim> p1, p2;
   p1 =
      Point<dim>(2 * half_length, -half_length, -half_length);
   p2 =
      Point<dim>(0, half_length, half_length );
...
pcout<<"setup dofs "<<std::endl;
  ...
  DoFTools::make_flux_sparsity_pattern(dof_handler,
                                       dsp);
pcout<<"Sparsity_before dist "  <<dsp.n_rows()<<" "<<dsp.n_cols()<<" n_nonzero_elements " <<dsp.n_nonzero_elements()<<std::endl;
